        ...
        <div class="row justify-content-center">
            <div class="col-lg-12">
                <div class="card shadow">
                    ...
                    <div class="card-body">
                        <form
                            action="{{ isset($plot) ? route('admin.plots.update', $plot->id) : route('admin.plots.store') }}"
                            method="POST">
                            @csrf
                            @if (isset($plot))
                                @method('PUT')
                                    ...
                                    <div class="mb-3">
                                        ...
                                        <select class="form-select" id="category_color" name="category_color" required>
                                            <option value="bg-primary"
                                                {{ old('category_color', $plot->category_color ?? '') == 'bg-primary' ? 'selected' : '' }}>
                                                Blue (bg-primary)</option>
                                            <option value="bg-success"
                                                {{ old('category_color', $plot->category_color ?? '') == 'bg-success' ? 'selected' : '' }}>
                                                Green (bg-success)</option>
                                            <option value="bg-warning"
                                                {{ old('category_color', $plot->category_color ?? '') == 'bg-warning' ? 'selected' : '' }}>
                                                Yellow (bg-warning)</option>
                                            <option value="bg-danger"
                                                {{ old('category_color', $plot->category_color ?? '') == 'bg-danger' ? 'selected' : '' }}>
                                                Red (bg-danger)</option>
                                            <option value="bg-info"
                                                {{ old('category_color', $plot->category_color ?? '') == 'bg-info' ? 'selected' : '' }}>
                                                Teal (bg-info)</option>
                                        </select>
                                    </div>
                                </div>
                                ...
                                <div class="mb-3">
                                    <div class="form-check">
                                        <input class="form-check-input" type="checkbox" id="is_active" name="is_active"
                                            value="1" {{ $plot->is_active ? 'checked' : '' }}>
